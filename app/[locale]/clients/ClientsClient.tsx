...
import type { ClientsInitialData } from '@/lib/clients/fetchClientsData.server';

// Lazy-loaded: the CRM import modal (and the xlsx parser it pulls in) is only
// needed when the user opens it, so it stays out of the initial bundle.
const CrmImportModal = dynamic(() => import('@/components/clients/CrmImportModal'), {
  ssr: false,
});
...
export default function ClientsClient({
  ...
  initialData: ClientsInitialData | null;
}) {
  ...
  // Data states — seeded from the server-rendered payload when available.
  ...
  useEffect(() => {
    // Server already seeded the initial data — don't re-fetch on mount.
